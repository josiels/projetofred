...
<div class="panel-heading"><strong>Lista de Alunos não inscritos</strong></div>
    ...
    <table class="table table-responsive table-hover">
        ...
        <tbody>
            ...
            <?php
                ...
                $cont = 0;
                if($_SESSION['nivel'] == 1){
                    #$sql = $pdo->prepare('SELECT DISTINCT login_usuario,nome_usuario,fk_id_turma,nome_turma FROM usuario INNER JOIN matricula INNER JOIN turma ON usuario.id_usuario != matricula.fk_id_usuario AND usuario.nivel_usuario = 0 LIMIT 10');
                    $sql = $pdo->prepare('SELECT DISTINCT login_usuario,nome_usuario,fk_id_turma FROM usuario INNER JOIN matricula ON usuario.id_usuario != matricula.fk_id_usuario AND usuario.nivel_usuario = 0 LIMIT 10');
                    $sql->execute();
...
<?php
  ...
  foreach ($resultado as $var){
    if($var->nivel == 1){
      $fundamental1[] = "<a class='list-group-item' href='listarturma.php?codturma=".$var->id_turma."'>".$var->nome_turm." - ".$var->nome."</a>";
    }else if($var->nivel == 2){
